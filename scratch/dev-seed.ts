...
const DEMO_LIMITS = {
  ...
  maxBid: { dollars: 100, tokens: 5_000_000, humanMinutes: 120, maxRounds: 10, maxHops: DEFAULT_LIMITS.maxBid!.maxHops!, maxDiskBytes: 8 * 1024 * 1024 * 1024 },
};
...
const cShip = await claim('infra', 'release:v2.4', 'v2.4 ships behind flag `v24`', 'FACT', 1, 'https://linear.net/rel/1');
const cLatency = await claim('infra', 'api:p95', 'p95 latency is 240ms', 'FACT', 0.98, 'https://grafana.net/d/api-p95');
const cChurn = await claim('business', 'churn:q3', 'Q3 churn holds under 2.1%', 'FACT', 0.94, 'https://stripe.com/atlas');
const cBudget = await claim('finance', 'budget:q4', 'Q4 infra budget is $48k', 'FACT', 0.99, 'https://netsuite.com/gl');
const cClaimRate = await claim('product', 'onboarding:dropoff', 'Onboarding drop-off is concentrated at step 3', 'HYPOTHESIS', 0.62, 'https://amplitude.com/cohort');
const cPricing = await claim('exec', 'pricing:seat', 'Seat-based pricing lifts net expansion', 'HYPOTHESIS', 0.55, 'https://docs.google.com/pricing-model');
const cContract = await claim('legal', 'dpa:eu', 'EU DPA revision clears counsel review', 'FACT', 1, 'https://docusign.com/env/1');
const cSupport = await claim('risk', 'support:backlog', 'Support backlog clears within 48h next quarter', 'PREDICTION', 0.71, 'https://zendesk.com/reports/1');
const cDesign = await claim('product', 'design:tokens', 'Design tokens ship in the same release as the console', 'FACT', 0.97, 'https://figma.com/file/tokens');
const cResearch = await claim('research', 'market:apac', 'APAC demand grows past EMEA in 2027', 'PREDICTION', 0.48, 'https://internal/reports/apac');
...
const s1 = await propose({ originScope: 'product', targetScope: 'infra', goal: 'ship console redesign', claimRefs: [cShip, cDesign] });
...
const s2 = await propose({ originScope: 'business', targetScope: 'data', goal: 'reconcile Q3 churn against Atlas', claimRefs: [cChurn] });
...
const s3 = await propose({ originScope: 'finance', targetScope: 'infra', goal: 'model Q4 infra budget against usage', claimRefs: [cBudget, cLatency] });
...
const s4 = await propose({ originScope: 'legal', targetScope: 'exec', goal: 'route the EU DPA revision to counsel', claimRefs: [cContract] });
...
const s5 = await propose({ originScope: 'product', targetScope: 'research', goal: 'size APAC demand for 2027 planning', claimRefs: [cResearch] });
...
await propose({ originScope: 'product', targetScope: 'infra', goal: 'cut onboarding drop-off at step 3', claimRefs: [cClaimRate], bid: { dollars: 5, humanMinutes: 30 } });
await propose({ originScope: 'exec', targetScope: 'finance', goal: 'validate seat-based pricing before the board call', claimRefs: [cPricing], bid: { dollars: 8, humanMinutes: 45 } });
await propose({ originScope: 'risk', targetScope: 'product', goal: 'clear the support backlog before renewal season', claimRefs: [cSupport], bid: { dollars: 3, humanMinutes: 20 } });
await propose({ originScope: 'research', targetScope: 'business', goal: 'pressure-test the APAC forecast assumptions', claimRefs: [cResearch], bid: { dollars: 4, humanMinutes: 15 } });
...
const s9 = await propose({ originScope: 'data', targetScope: 'legal', goal: 'export raw customer rows for a one-off analysis', claimRefs: [cChurn], bid: { dollars: 2, humanMinutes: 10 } });
...
await say('general', 'human:owner', 'Morning — the console redesign is on staging. Please flag anything that reads wrong.', '08:45');
...
await say('risk', 'agent:risk', 'Support backlog is the renewal risk. 48h clearance is a prediction, not a fact yet.', '11:48');
await say('product', 'agent:product', 'Onboarding drop-off at step 3 is still a hypothesis — the cohort query is not conclusive.', '12:05');
...
await createCustomRoom(
  db,
  TENANT,
  { id: 'design', name: 'design', scope: 'design', agentName: 'design-agent', mission: 'Design system reviews for the console.' },
  'human:owner',
);
...
const counts = {
  ...
  escalations: (await db.prepare('SELECT COUNT(*) AS n FROM escalations WHERE tenant = ?').get(TENANT)) as { n: number },
  messages: (await db.prepare('SELECT COUNT(*) AS n FROM buzz_messages WHERE tenant = ?').get(TENANT)) as { n: number },
  spend: (await db.prepare('SELECT COALESCE(SUM(spent_dollars),0) AS d FROM requests WHERE tenant = ?').get(TENANT)) as { d: number },
};
...
console.log(`  limits      $${DEMO_LIMITS.maxDailyDollars}/day, ${DEMO_LIMITS.maxHumanEscalationsPerDay} escalations/day (demo, not production defaults)`);
console.log('');
