 ...
 *   upstream path: src/policy/command-policy.ts (912 lines)
 *   license:       MIT — see LICENSE-THIRD-PARTY.md
 *
 * WHAT WAS CHANGED AND WHY
 *   Upstream `../types.ts` import replaced with narrowed local type defs (only CommandDecision/CommandPolicy/CommandRule are read; CommandPolicyMode widened to exported).
 *   Upstream `../util/errors.ts` and `../util/safe-regex.ts` imports re-pointed at the verbatim-vendored `./errors.ts` and `./safe-regex.ts`.
 *   No logic line changed: rule lists, shell scanner, SQL payload extraction, and first-match evaluation are byte-identical.
 *
...
/**
 * Narrowed upstream command types (src/types.ts, same commit), copied
 * field-for-field. One deliberate widening: CommandPolicyMode is exported
 * here (it is module-private upstream) because our own policy code types
 * against it.
 */
export type CommandDecision = "allow" | "deny" | "require_approval";
export type CommandPolicyMode = "denylist" | "allowlist";
