...
export interface LiveEvent { seq: number; action: string; target: string; detail: string; at: string; actor: string; }
...
export function handleEventStream(req: IncomingMessage, res: ServerResponse, db: AsyncDb, tenant: string): void {
  ...
  const poll = async (): Promise<void> => {
    ...
    try {
      ...
      for (const r of rows) { send(r); since = r.seq; }
    } catch { /* next tick retries; stream stays open */ }
  };
  ...
  setTimeout(() => { if (!closed) { closed = true; clearInterval(timer); res.end(); } }, 5 * 60 * 1000).unref?.();
  res.write(': connected\n\n');
  void poll();
}
