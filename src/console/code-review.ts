...
// Every rule is scoped under `.rvw`, the wrapper `reviewDocument` emits. The
// surface used to return a whole HTML document — it was reachable only by typing
// its URL — so its rules were written as if they owned the page: a bare `body`
// rule, a bare `.card`, a bare `pre`. Inside the console shell that is a page
// repainting chrome it does not own, which is exactly the class of bug the shell
// exists to prevent. Scoping costs a prefix and makes the collision impossible.
//
// This surface used to carry its own palette (--ink/--teal/--bg + raw hex) too.
// It now reads the token sheet, so a theme change or a dark-mode fix lands here
// without editing review code: the local names are aliases, none is a literal.
const CSS = `
.rvw{--ink:var(--v-ink);--teal:var(--v-accent);--bg:var(--v-bg-0);--line:var(--v-line);--add:var(--v-tint-good-bg);--add-b:var(--v-fact);--del:var(--v-tint-risk-bg);--del-b:var(--v-risk);--mono:var(--font-mono);font-family:var(--font-body);color:var(--ink);font-size:14px}
.rvw .top{border-bottom:1px solid var(--line);background:var(--v-bg-1);padding:12px 16px;display:flex;gap:16px;align-items:center;flex-wrap:wrap}
.rvw .steps{display:flex;gap:6px;font-size:12px;color:var(--v-muted)}.rvw .steps b{color:var(--teal)}.rvw .steps .done{color:var(--v-fact)}
.rvw .layout{display:grid;grid-template-columns:minmax(0,250px) minmax(0,1fr) minmax(0,280px);gap:0}
@media(max-width:1100px){.rvw .layout{grid-template-columns:minmax(0,220px) minmax(0,1fr)}.rvw .side-r{display:none}}
@media(max-width:760px){.rvw .layout{grid-template-columns:minmax(0,1fr)}.rvw .side-l{max-height:200px;overflow:auto}}
.rvw .side-l{border-right:1px solid var(--line);background:var(--v-bg-1);padding:12px;overflow:auto}
.rvw .side-r{border-left:1px solid var(--line);background:var(--v-bg-1);padding:12px;overflow:auto;font-size:13px}
.rvw .main{padding:12px 16px;overflow:auto;min-width:0}
.rvw .diff{display:grid;grid-template-columns:minmax(0,1fr) minmax(0,1fr);border:1px solid var(--line);border-radius:8px;overflow:hidden;background:var(--v-bg-1)}
.rvw .diff.inline{grid-template-columns:minmax(0,1fr)}
.rvw .pane{overflow:auto;min-width:0}.rvw .pane+.pane{border-left:1px solid var(--line)}
.rvw .pane h4{margin:0;padding:8px 12px;background:var(--v-bg-2);font-size:12px;position:sticky;top:0}
.rvw pre{margin:0;padding:8px 0;font-family:var(--mono);font-size:12.5px;line-height:1.6}
.rvw .ln{display:flex;min-width:0}.rvw .ln .no{flex:0 0 44px;text-align:right;padding-right:10px;color:var(--v-faint);user-select:none}
.rvw .ln .tx{white-space:pre-wrap;word-break:break-word;padding-right:12px;flex:1}
.rvw .ln.add{background:var(--add)}.rvw .ln.del{background:var(--del)}
.rvw .ln .mk{font-weight:700;color:var(--add-b)}.rvw .ln.del .mk{color:var(--del-b)}
.rvw .hunk{border-bottom:1px solid var(--line)}.rvw .hunk-bar{display:flex;gap:8px;align-items:center;padding:6px 10px;background:var(--v-bg-2);font-size:12px;position:sticky}
.rvw .hunk-bar form{display:inline}.rvw .btn{padding:4px 10px;border:1px solid var(--line);border-radius:6px;background:var(--v-bg-1);cursor:pointer;font-size:12px;font-weight:600;color:var(--ink)}
.rvw .btn.pri{background:var(--teal);color:var(--v-accent-ink);border-color:var(--teal)}.rvw .btn.dan{color:var(--del-b)}.rvw .btn:hover{filter:brightness(.96)}
.rvw .file-row{display:flex;gap:6px;align-items:center;padding:5px 8px;border-radius:6px;font-family:var(--mono);font-size:12.5px;color:var(--ink);text-decoration:none}
.rvw .file-row:hover{background:var(--v-bg-2)}.rvw .file-row.sel{background:var(--v-accent-dim)}.rvw .st{font-weight:700;width:16px}.rvw .st.M{color:var(--v-hypo)}.rvw .st.A{color:var(--v-fact)}.rvw .st.D{color:var(--v-risk)}.rvw .st.R{color:var(--v-pred)}
.rvw .card{border:1px solid var(--line);border-radius:8px;background:var(--v-bg-1);padding:12px;margin-bottom:12px}
.rvw .meter{display:flex;gap:14px;flex-wrap:wrap;font-size:13px}.rvw .ok{color:var(--v-fact)}.rvw .bad{color:var(--v-risk)}.rvw .mut{color:var(--v-muted)}
...
.rvw .tk-k{color:var(--v-pred)}.rvw .tk-s{color:var(--v-fact)}.rvw .tk-c{color:var(--v-faint);font-style:italic}.rvw .tk-n{color:var(--v-hypo)}
.rvw textarea.code{width:100%;min-height:300px;font-family:var(--mono);font-size:12.5px;border:1px solid var(--line);border-radius:8px;padding:10px}
.rvw .tabs{display:flex;gap:8px;margin:12px 0}.rvw .term{background:var(--v-code-bg);color:var(--v-code-ink);border-radius:8px;padding:12px;font-family:var(--mono);font-size:12px;white-space:pre-wrap;max-height:320px;overflow:auto}
.rvw table.meta{border-collapse:collapse;width:100%;font-size:12.5px}.rvw table.meta td{border-bottom:1px solid var(--line);padding:5px 4px;vertical-align:top}
`;

/**
 * The review surface's styles, markup wrapper and interactions, as a page *body*.
 *
 * It returns a body rather than a document because the console shell owns the
 * document now — this page is reached from the review index and from the
 * Agent Tasks row that opened it, and a page that arrives with its own `<html>`
 * cannot carry the rail the reader navigated with. The `<script>` is the part
 * that must survive the move: pane scroll sync, hunk jumping, the unsaved-changes
 * guard and the confirm dialog are the whole interaction of a review.
 */
function reviewDocument(inner: string): string {
  return `<style>${CSS}</style><div class="rvw">${inner}</div>
<script>
...
/** Diff-row class for one line type: added, deleted, or context. */
...
/** The marker in the gutter. U+2212 (minus sign) for a deletion, not a hyphen. */
function lineMark(type: string): string {
  if (type === '+') return '+';
  if (type === '-') return '\u2212';
  return ' ';
}

/** `M`/`A`/`D` spelled out. A rename is the fallback, which is the only other
 * letter git porcelain reports here. */
...
function hunkHtml(
  ...
  if (mode === 'inline') {
    for (const l of h.lines) {
      const cls = lineClass(l.type);
      const mk = lineMark(l.type);
    ...
    if (f.status === 'A')
      left = `<div class="ln"><span class="no"></span><span class="tx mut">No file existed (new file).</span></div>`;
    ...
    if (f.status === 'D')
      right = `<div class="ln"><span class="no"></span><span class="tx mut">FILE DELETED — restoring the baseline means rejecting the whole set (Reject All below); a single deleted file cannot be restored on its own.</span></div>`;
  }
...
export async function renderReviewPage(
  ...
  if (!doc) {
    ...
    return reviewDocument(`<div class="top"><span style="font-family:var(--mono)">${esc(missionId)}</span><span class="mut">no review opened</span></div>
...
<button class="btn pri" type="submit">Open review</button></form></div></main>`);
  ...
  const stepHtml = steps
    .map((t) => {
      if (t === 'Code Review') return `<b>● ${t}</b>`;
      if (t === 'Snapshot') return `<span>○ ${t}</span>`;
      return `<span class="done">✓ ${t}</span>`;
    })
    .join(' · ');
  ...
  for (const [g, fs] of groups) {
    ...
    for (const f of fs)
      sidebar += `<a class="file-row${sel?.id === f.id ? ' sel' : ''}" href="?file=${f.id}&mode=${mode}${q.q ? `&q=${esc(q.q)}` : ''}"><span class="st ${f.status}">${f.status}</span><span>${esc(f.path)}${f.humanModified ? ' ●' : ''}</span></a>`;
  }
  if (files.length === 0) sidebar += `<p class="mut">No changes vs baseline.</p>`;

  let main = '';
  if (!sel)
    main = `<div class="card"><p class="mut">No files match. The working tree equals the baseline${q.q ? ` for filter “${esc(q.q)}”` : ''}.</p></div>`;
  else {
    const lang = languageForPath(sel.path);
    const selHits = secretHits.get(sel.path) ?? [];
    main += `<div class="card"><b style="font-family:var(--mono)">${esc(sel.path)}</b> <span class="st ${sel.status}">${sel.status} ${statusText(sel.status)}</span>
...
<button class="btn pri" type="submit">Save to working tree</button> <a class="btn" href="?file=${sel.id}&mode=${mode}">Cancel</a></form></div>`;
  ...
  const ver =
    doc.verification
      ...
      .map(
        (v) =>
          `<div>${v.ok ? '<span class="ok">✓</span>' : '<span class="bad">✗</span>'} <b>${esc(v.suite)}</b> ${v.passed}/${v.passed + v.failed} <span class="mut">${esc(v.at)}</span><div class="term">${esc(v.output.slice(0, 2000))}</div></div>`,
      )
      .join('') || '<p class="mut">Not run</p>';
...
<div class="card"><h3>Timeline</h3>${doc.iterations.map((i) => `<div>v${i.n} ${esc(i.label)} <span class="mut">${esc(i.at)} · ${i.files}f +${i.insertions} −${i.deletions}</span></div>`).join('') || '<p class="mut">Review v1 — agent implementation.</p>'}${doc.humanEdits
    .slice(-6)
    .map((e) => `<div>✎ ${esc(e.file)} <span class="mut">${esc(e.author)} ${esc(e.at)}</span></div>`)
    .join('')}</div>
...
${!doc.snapshotId ? `<form method="post"><input type="hidden" name="csrf" value="${esc(csrf)}"><input type="hidden" name="action" value="create-snapshot"><button class="btn pri" type="submit">Create Verified Snapshot</button></form>` : `<a class="btn" href="?file=${sel?.id ?? ''}">New task from snapshot</a>`}</div>`;
  ...
  // No wordmark: the console shell above this body is the brand now, and two of
  // them read as a page that does not know it moved. What stays is the part that
  // is about *this* review — its mission, its meter, and its bulk decisions.
  const body = `${noticeHtml}<div class="top"><span style="font-family:var(--mono)">${esc(doc.missionId)}</span><b class="ok">✓ READY FOR REVIEW</b> ${dirty}
...
export async function handleReviewAction(
  ...
  switch (action) {
    ...
    case 'hunk-reject': {
      ...
      return {
        redirect: at(action === 'hunk-accept' ? 'Hunk accepted.' : 'Hunk rejected — file will be restored on save.'),
      };
    }
    case 'file-accept':
    case 'file-reject': {
    ...
    case 'reject-all': {
      ...
      for (const f of files) {
        ...
        const restored = applyHunkDecisions(
          original,
          cur,
          f.hunks.map((h) => ({ ...h, decision: 'rejected' as const })),
        );
        if (restored === null) {
      ...
      await recordIteration(db, tenant, missionId, 'reject-all — baseline restored', {
        files: files.length,
        insertions: 0,
        deletions: 0,
      });
    ...
    case 'run-tests': {
      ...
      await recordVerification(db, tenant, missionId, {
        suite: 'npm test',
        passed,
        failed,
    ...
    case 'create-snapshot': {
      ...
      const snap = await createSnapshot(db, tenant, {
        ...
        groupId: 'review',
        projectId: missionId,
        repo: basename(doc.workdir),
        branch: 'review',
        commit: doc.baselineRev,
        parentId: null,
        type: 'VERIFIED',
        runtime: {},
        docker: [],
        toolchains: {},
        workspaceContent: 'review-gated',
      });
