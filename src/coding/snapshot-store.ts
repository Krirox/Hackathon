...
export interface SnapshotBlobRef {
  hash: string;
  ref: string;
  bytes: number;
  deduped: boolean;
...
export class FsSnapshotStore implements SnapshotStore {
  ...
  private async indexRef(hash: string, ref: string): Promise<void> {
    ...
    await this.db
      ...
      .run(`blobidx:${this.tenant}:${hash}`, ref)
      .catch(() => {});
  }
  async put(kind: string, body: string | Buffer): Promise<SnapshotBlobRef> {
    ...
    if (this.db && this.tenant) {
      const hit = (await this.db
        ...
        .get(`blob:${this.tenant}:${kind}:${hash}`)) as { value: string } | undefined;
      if (hit) {
    ...
    if (this.db && this.tenant) {
      await this.db
        .prepare('INSERT INTO meta (key, value) VALUES (?, ?) ON CONFLICT(key) DO UPDATE SET value = excluded.value')
        .run(`blob:${this.tenant}:${kind}:${hash}`, ref)
        .catch(() => {});
    }
    await this.indexRef(hash, ref);
  ...
  async get(hash: string): Promise<Buffer | null> {
    ...
    const row = (await this.db
      .prepare('SELECT value FROM meta WHERE key = ?')
      .get(`blobidx:${this.tenant}:${hash}`)) as { value: string } | undefined;
    if (!row) return null;
    return this.fs.get(String(row.value));
  }
  async getByRef(ref: string): Promise<Buffer> {
    return this.fs.get(ref);
  }
}
...
export async function persistSnapshotLayers(
  store: SnapshotStore,
  o: { workspace: string; dependencies: string; dockerDigests: string[] },
