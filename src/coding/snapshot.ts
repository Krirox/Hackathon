...
export function stripSecrets<T>(
  obj: T,
  secretKeys = ['key', 'token', 'secret', 'password', 'credential'],
): { clean: T; refs: string[] } {
  const refs: string[] = [];
...
export async function createSnapshot(
  ...
  input: Omit<
    SnapshotManifest,
    'id' | 'status' | 'workspaceHash' | 'envHash' | 'manifestHash' | 'createdAt' | 'pinned' | 'secretRefs' | 'createdBy'
  > & { createdBy?: string; secretRefs?: string[]; workspaceContent?: string },
  ...
  const manifestHash = createHash('sha256')
    .update(workspaceHash + envHash + (rest.parentId ?? ''))
    .digest('hex');
  const s: SnapshotManifest = {
    ...rest,
    createdBy: rest.createdBy ?? tenant,
    secretRefs: rest.secretRefs ?? [],
  ...
  await db
    ...
    .run(
      ...
      'SNAPSHOT_CREATED',
      s.id,
      `${s.type} parent=${s.parentId ?? 'none'} hash=${manifestHash.slice(0, 12)}`,
      s.createdAt,
    )
    .catch(() => {});
  return s;
...
export async function getSnapshot(db: AsyncDb, tenant: string, id: string): Promise<SnapshotManifest | null> {
  const row = (await db.prepare('SELECT value FROM meta WHERE key = ?').get(`snapshot:${tenant}:${id}`)) as
...
export async function setSnapshotStatus(
  ...
  await db
    ...
    .run(
      tenant,
      'coding-agent',
      status === 'VERIFIED' ? 'SNAPSHOT_VERIFIED' : 'SNAPSHOT_STATUS',
      id,
      status,
...
export function checkCompatibility(
  ...
  req: { repo: string; runtime: Record<string, string> },
  ...
  for (const [k, v] of Object.entries(req.runtime)) {
    if (snap.runtime[k] && snap.runtime[k] !== v)
      return { ok: false, reason: `runtime ${k}: ${snap.runtime[k]} vs ${v}` };
  }
...
export async function listSnapshots(db: AsyncDb, tenant: string): Promise<SnapshotManifest[]> {
  const rows = (await db.prepare('SELECT value FROM meta WHERE key LIKE ?').all(`snapshot:${tenant}:%`)) as {
    value: string;
  }[];
  return rows
    .map((r) => JSON.parse(String(r.value)) as SnapshotManifest)
...
export async function selectBestSnapshot(
  ...
  const compat = snaps
    .filter((s) => checkCompatibility(s, req).ok)
...
export async function saveMemory(
  db: AsyncDb,
  tenant: string,
  mem: Omit<MissionMemory, 'version' | 'updatedAt'> & { version?: number },
...
export async function getMemory(db: AsyncDb, tenant: string, missionId: string): Promise<MissionMemory | null> {
  const row = (await db.prepare('SELECT value FROM meta WHERE key = ?').get(`memory:${tenant}:${missionId}`)) as
    { value: string } | undefined;
