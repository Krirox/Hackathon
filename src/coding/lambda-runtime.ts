...
export interface LambdaSession {
  vmId: string; workdir: string; kind: 'lambda';
  baseSnapshotId: string | null; functionName: string;
  timeoutMs: number; ephemeral: true;
}
...
export async function restoreSnapshotToLambda(
  db: AsyncDb, tenant: string, snap: SnapshotManifest, vmId: string,
  ...
  await db.prepare('INSERT INTO audit_log (tenant, actor, action, target, detail, at) VALUES (?,?,?,?,?,?)')
    .run(tenant, 'microvm-lambda', 'SNAPSHOT_RESTORE_COMPLETED', snap.id, `vm=${vmId} dir=${workdir} envHash=${snap.envHash.slice(0, 12)}`, new Date().toISOString()).catch(() => {});
  return { vmId, workdir, kind: 'lambda', baseSnapshotId: snap.id, functionName: lambdaFunctionName(), timeoutMs: 14 * 60 * 1000, ephemeral: true };
}
...
export function buildLambdaPayload(o: {
  tenant: string; missionId: string; groupId: string; taskIds: string[];
  snapshotId: string | null; continuationPrompt: string; opts?: LambdaLaunchOpts;
}): Record<string, unknown> {
  return {
    tenant: o.tenant, missionId: o.missionId, groupId: o.groupId, taskIds: o.taskIds,
    baseSnapshotId: o.snapshotId, prompt: o.continuationPrompt,
    functionName: lambdaFunctionName(),
    timeoutMs: o.opts?.timeoutMs ?? 14 * 60 * 1000,
    ephemeral: true, durableState: 'snapshot-store-only',
  };
}
