...
const TRANSITIONS: Record<MissionStatus, MissionStatus[]> = {
  ...
  VERIFYING: ['SNAPSHOTTING', 'FAILED'],
  SNAPSHOTTING: ['COMPLETE', 'FAILED'],
  COMPLETE: [],
...
async function emit(db: AsyncDb, tenant: string, type: string, ref: string, detail = ''): Promise<void> {
  ...
  await db
    ...
    .run(tenant, 'coding-agent', type, ref, detail.slice(0, 500), at)
    .catch(() => {});
}
...
export async function getMission(db: AsyncDb, tenant: string, id: string): Promise<Mission | null> {
  const row = (await db.prepare('SELECT value FROM meta WHERE key = ?').get(`mission:${tenant}:${id}`)) as
...
function compatible(a: TaskSpec, b: TaskSpec): string | null {
  ...
  for (const [k, v] of Object.entries(a.runtime))
    if (b.runtime[k] && b.runtime[k] !== v) return `runtime conflict ${k}`;
