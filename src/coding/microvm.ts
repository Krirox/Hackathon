...
export function resolveRuntime(): RuntimeKind {
  ...
  return (['local', 'firecracker', 'lambda', 'fargate', 'container', 'vm'] as RuntimeKind[]).includes(v as RuntimeKind)
    ? (v as RuntimeKind)
    : 'local';
}
async function emit(db: AsyncDb, tenant: string, type: string, ref: string, detail = ''): Promise<void> {
  await db
    ...
    .run(tenant, 'microvm', type, ref, detail.slice(0, 300), new Date().toISOString())
    .catch(() => {});
}
export async function createMicroVM(
  db: AsyncDb,
  tenant: string,
  missionId: string,
  groupId: string,
  ...
  if (kind === 'lambda') {
    ...
    await emit(
      db,
      tenant,
      baseSnapshotId ? 'MICROVM_RESTORED' : 'MICROVM_CREATED',
      m.id,
      `kind=lambda fn=${lambdaFunctionName()} group=${groupId} base=${baseSnapshotId ?? 'none'} eph=/tmp`,
    );
    return m;
  }
  if (kind === 'fargate') {
    ...
    if (baseSnapshotId && !snap)
      await emit(db, tenant, 'SNAPSHOT_RESTORE_FAILED', baseSnapshotId, `vm=${id} reason=not-found`);
  ...
  await emit(
    ...
    baseSnapshotId ? 'MICROVM_RESTORED' : 'MICROVM_CREATED',
    m.id,
    `kind=${kind} group=${groupId} base=${baseSnapshotId ?? 'none'}`,
  );
  return { ...m, id: m.id };
}
export async function stopMicroVM(
  db: AsyncDb,
  tenant: string,
  groupId: string,
  missionId: string,
...
export function renderSnapshotList(snaps: { id: string; status: string; missionId: string }[]): string {
  const rows = snaps
    .map((s) => `<li>${s.id} — ${s.status} (${s.missionId}) [INSPECT] [RESTORE] [BRANCH]</li>`)
    .join('');
