...
export interface MergeOutcome {
  ...
  mergedFiles: string[];
  conflicts: { file: string; groups: string[] }[];
  reason: string;
}
const HIGH_RISK = [/migrate/i, /schema/i, /auth/i, /ledger/i, /gov\//i];
function isHighRisk(f: string): boolean {
  return HIGH_RISK.some((r) => r.test(f));
}
...
async function emit(db: AsyncDb, tenant: string, type: string, ref: string, detail = ''): Promise<void> {
  await db
    ...
    .run(tenant, 'merge-manager', type, ref, detail.slice(0, 400), new Date().toISOString())
    .catch(() => {});
}
export async function mergeGroups(
  ...
  await emit(
    ...
    'MERGE_STARTED',
    missionId,
    `branches=${plan.branches.map((b) => b.groupId).join(',')} base=${plan.baseSnapshotId}`,
  );
  ...
  const conflicts = [...owners.entries()].filter(([, g]) => g.length > 1).map(([file, groups]) => ({ file, groups }));
  ...
  await emit(
    ...
    'MERGE_COMPLETED',
    missionId,
    `files=${mergedFiles.length} from ${plan.branches.length} branches`,
  );
