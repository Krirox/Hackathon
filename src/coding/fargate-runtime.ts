...
async function emit(db: AsyncDb, tenant: string, type: string, ref: string, detail = ''): Promise<void> {
  await db
    ...
    .run(tenant, 'microvm-fargate', type, ref, detail.slice(0, 300), new Date().toISOString())
    .catch(() => {});
}
export async function launchFargate(
  ...
  tenant: string,
  vmId: string,
  o: { missionId: string; groupId: string; taskIds: string[]; continuationPrompt: string },
  ...
  if (snap)
    writeFileSync(
      join(workdir, 'snapshot-manifest.json'),
      JSON.stringify({ ...snap, restoredAt: new Date().toISOString() }),
    );
  ...
  await emit(
    db,
    tenant,
    snap ? 'MICROVM_RESTORED' : 'MICROVM_CREATED',
    vmId,
