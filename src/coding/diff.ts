...
export interface DiffLine { type: ' ' | '-' | '+'; text: string; origNo: number | null; newNo: number | null; }
export interface DiffHunk {
  id: string; origStart: number; origLength: number; newStart: number; newLength: number;
  lines: DiffLine[]; changeType: 'add' | 'del' | 'mod';
  agentId: string | null; planStepId: string | null; eventId: string | null; timestamp: string | null;
  decision: 'pending' | 'accepted' | 'rejected';
}
export interface ChangedFile {
  id: string; path: string; status: FileStatus; oldPath: string | null;
  originalHash: string; currentHash: string;
  insertions: number; deletions: number;
  hunks: DiffHunk[]; group: string;
  agentIds: string[]; humanModified: boolean;
}
export function sha(s: string): string { return createHash('sha256').update(s).digest('hex'); }
export function groupForPath(p: string): string {
  ...
  if (/(^|\/)(components?|pages?|views?|app|web|site|frontend|ui)\//.test(l) || /\.(tsx|jsx|vue|css|scss)$/.test(l)) return 'Frontend';
  if (/(^|\/)(docker|deploy|infra|k8s|terraform|workflows\/)\//.test(l) || /dockerfile|compose|\.ya?ml$/.test(l)) return 'Infrastructure';
...
export function languageForPath(p: string): string {
  ...
  const m: Record<string, string> = {
    ...
    '.py': 'python', '.go': 'go', '.rs': 'rust', '.json': 'json', '.yaml': 'yaml', '.yml': 'yaml',
    '.md': 'markdown', '.sql': 'sql', '.html': 'html', '.css': 'css', '.sh': 'bash', '.tf': 'hcl',
  };
  return m[e] ?? 'text';
}
...
export function diffLines(orig: string[], cur: string[], context = 3): { hunks: Omit<DiffHunk, 'id' | 'decision' | 'agentId' | 'planStepId' | 'eventId' | 'timestamp'>[] } {
  let a = 0;
  while (a < orig.length && a < cur.length && orig[a] === cur[a]) a++;
  let bO = orig.length - 1, bC = cur.length - 1;
  while (bO >= a && bC >= a && orig[bO] === cur[bC]) { bO--; bC--; }
  const O = orig.slice(a, bO + 1), C = cur.slice(a, bC + 1);
  // LCS table on the differing middle only.
  const n = O.length, m = C.length;
  const ops: ('=' | '-' | '+')[] = [];
  if (n * m > 4_000_000) {
    ...
    for (let i = n - 1; i >= 0; i--) for (let j = m - 1; j >= 0; j--) dp[i]![j] = O[i] === C[j] ? (dp[i + 1]![j + 1]! + 1) : Math.max(dp[i + 1]![j]!, dp[i]![j + 1]!);
    let i = 0, j = 0;
    while (i < n && j < m) {
      if (O[i] === C[j]) { ops.push('='); i++; j++; }
  ...
  let oi = 0, ci = 0;
  for (const op of ops) {
    if (op === '=') { full.push({ t: '=', o: O[oi]! }); oi++; ci++; }
    else if (op === '-') { full.push({ t: '-', o: O[oi]! }); oi++; }
    else { full.push({ t: '+', o: C[ci]! }); ci++; }
  }
  ...
  for (let k = 1; k < changedIdx.length; k++) {
    if (changedIdx[k]! - changedIdx[k - 1]! > 2 * context + 1) { clusters.push(cur2); cur2 = [changedIdx[k]!]; }
  ...
  for (const cl of clusters) {
    ...
    let o = 1, nn = 1;
    for (let k = 0; k < start; k++) { if (full[k]!.t !== '+') o++; if (full[k]!.t !== '-') nn++; }
    const lines: DiffLine[] = [];
    let oL = 0, nL = 0, adds = 0, dels = 0;
    for (let k = start; k <= end; k++) {
      const f = full[k]!;
      if (f.t === '=') { lines.push({ type: ' ', text: f.o, origNo: o, newNo: nn }); o++; nn++; oL++; nL++; }
      else if (f.t === '-') { lines.push({ type: '-', text: f.o, origNo: o, newNo: null }); o++; oL++; dels++; }
      else { lines.push({ type: '+', text: f.o, origNo: null, newNo: nn }); nn++; nL++; adds++; }
    }
    hunks.push({
...
export function diffFile(path: string, original: string | null, current: string | null, provenance?: Map<string, { agentId: string; planStepId: string; eventId: string; timestamp: string }>): ChangedFile {
  ...
  const out: DiffHunk[] = hunks.map((h, i) => {
    for (const l of h.lines) { if (l.type === '+') ins++; if (l.type === '-') del++; }
    const key = `${path}:${h.origStart}-${h.origStart + h.origLength}`;
    const p = provenance?.get(key);
    return {
      ...h, id: `h_${sha(path + i).slice(0, 10)}`,
      agentId: p?.agentId ?? null, planStepId: p?.planStepId ?? null, eventId: p?.eventId ?? null, timestamp: p?.timestamp ?? null,
      decision: 'pending' as const,
    };
  });
  return {
    id: `f_${sha(path).slice(0, 12)}`, path, status, oldPath: null,
    originalHash: original === null ? '' : sha(original), currentHash: current === null ? '' : sha(current),
    insertions: ins, deletions: del, hunks: out, group: groupForPath(path),
    agentIds: [...new Set(out.map((h) => h.agentId).filter(Boolean) as string[])], humanModified: false,
  };
}
...
export function applyHunkDecisions(original: string | null, current: string | null, hunks: DiffHunk[]): string | null {
  ...
  for (const h of sorted) {
    while (cursor < h.origStart && cursor <= oLines.length) { out.push(oLines[cursor - 1]!); cursor++; }
    if (h.decision === 'rejected') for (const l of h.lines) { if (l.type !== '+') out.push(l.text); }
    else for (const l of h.lines) { if (l.type !== '-') out.push(l.text); }
    cursor = h.origStart + h.origLength;
  }
  while (cursor <= oLines.length) { out.push(oLines[cursor - 1]!); cursor++; }
  return out.join('\n');
}
// ── Workspace readers ──
export function isGitRepo(dir: string): boolean {
  try { execFileSync('git', ['-C', dir, 'rev-parse', '--git-dir'], { stdio: 'pipe' }); return true; } catch { return false; }
}
export interface RepoChange { path: string; status: FileStatus; oldPath: string | null; }
export function gitStatus(dir: string): RepoChange[] {
  try {
    const out = execFileSync('git', ['-C', dir, 'status', '--porcelain', '-z'], { encoding: 'utf8', maxBuffer: 16 * 1024 * 1024 });
...
export function gitShow(dir: string, rev: string, path: string): string | null {
  try {
    const out = execFileSync('git', ['-C', dir, 'show', `${rev}:${path}`], { encoding: 'utf8', maxBuffer: 64 * 1024 * 1024, stdio: ['pipe', 'pipe', 'pipe'] });
...
export function readTree(dir: string, paths: string[]): Map<string, string | null> {
  ...
  for (const p of paths) {
    ...
    try { m.set(p, statSync(full).isFile() ? readFileSync(full, 'utf8') : null); } catch { m.set(p, null); }
  }
  return m;
}
export function listFilesRecursive(dir: string, base = dir, out: string[] = [], cap = 5000): string[] {
  ...
  for (const e of entries) {
    ...
    try {
      ...
      else { out.push(relative(base, full)); if (out.length >= cap) return out; }
    } catch { /* skip */ }
  }
  return out;
}
...
export function computeReviewFromGit(dir: string, baselineRev: string): ChangedFile[] {
  ...
  for (const c of changes) {
    ...
    if (c.status !== 'D') {
      try { cur = readFileSync(join(dir, c.path), 'utf8'); } catch { cur = null; }
    }
...
export function newReviewId(): string { return `RVW_${randomUUID().slice(0, 8).toUpperCase()}`; }
