...
const KEYWORDS: Record<string, string[]> = {
  typescript: 'const let var function return if else for while class interface type extends implements import export from new await async try catch throw switch case break continue typeof instanceof enum public private readonly void null undefined true false this super static get new'.split(' '),
  ...
  javascript: 'const let var function return if else for while class extends import export from new await async try catch throw switch case break continue typeof instanceof null undefined true false this super static'.split(' '),
  ...
  python: 'def return if elif else for while in not and or is None True False class import from as with try except raise lambda pass yield async await self'.split(' '),
...
export function highlightLine(line: string, lang: string): string {
  ...
  if (['typescript', 'tsx', 'javascript', 'jsx', 'go', 'rust'].includes(lang) && trimmed.startsWith('//')) return `<span class="tk-c">${esc}</span>`;
